...
Open the SQL tab and paste this code:
...
http://localhost/PESOoffice_db/index.php 
If it loads, test inserting data. To check saved data:

http://localhost/phpmyadmin

→ peso_database → ofw_profiles → Browse

6. Functions of each bat file (automation)

   a. SEND-IP - only MAIIN PC (HOST) needs this file
               - double click. It redirects you to chrome gmail and prompts sending the URL for other PC users
   b. xampp_backup - double click this to save backup file of database (sql format)
   c. Sync-Database - double click this once you've downloaded the updated csv file from Google forms to update the database
   
7. SETUP local backup
...
NOTES: IF main PC requires reformatting then make sure to save local backup sql to another device or USB drive
